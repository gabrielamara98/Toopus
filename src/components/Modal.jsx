...
function Modal({open, setOpen, id, nome, foto, cargo, resumo, localizacao, area, habilidadesTecnicas, softSkills, experiencias, formacao, projetos, certificacoes, idiomas, areaInteresses,
onConnectionChange,}) {
  ...
  return (
    <div>
      <Dialog open={open} onClose={setOpen} className="relative z-10">
        <DialogBackdrop transition className="fixed inset-0 bg-gray-900/60 transition-opacity data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in"/>

        <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            <DialogPanel transition className="relative transform overflow-hidden rounded-3xl bg-white dark:bg-gray-900 text-left text-gray-900 dark:text-gray-100 shadow-2xl transition-all data-closed:translate-y-4 data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in sm:my-8 sm:w-full sm:max-w-3xl data-closed:sm:translate-y-0 data-closed:sm:scale-95">
              <div className="px-6 py-6 sm:px-8 sm:py-8 dark:bg-gray-900">
                <div className="flex flex-col sm:flex-row gap-8">
                  <div className="flex flex-col items-center sm:items-start gap-4 sm:w-1/3">
                    ...
                    <div className="text-center sm:text-left">
                      <DialogTitle as="h3" className="text-xl font-semibold text-gray-900 dark:text-gray-100">{nome}</DialogTitle>
                      <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">{cargo}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{localizacao}</p>
                    ...
                    {habilidadesTecnicas && habilidadesTecnicas.length > 0 && (
                      <div className="flex flex-wrap justify-center sm:justify-start gap-2 mt-2">
                        {habilidadesTecnicas.map((habilidade) => (
                          <span key={habilidade} className="px-3 py-1 rounded-full bg-green-100 text-green-700 border border-green-300 text-xs dark:bg-green-900/40 dark:text-green-300 dark:border-green-500">{habilidade}</span>
                        ))}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="sm:w-2/3 max-h-[60vh] overflow-y-auto pr-1 space-y-4">
                    {resumo && (
                      <section>
                        <h4 className="text-xl font-semibold text-gray-800 dark:text-gray-100 mb-1">Sobre</h4>
                        <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{resumo}</p>
                      </section>
                    )}

                    {softSkills && softSkills.length > 0 && (
                      <section>
                        <h4 className="text-xl font-semibold text-gray-800 dark:text-gray-100 mb-1">Soft skills</h4>
                        <ul className="flex flex-wrap gap-2">
                          {softSkills.map((soft) => (
                            <li key={soft} className="px-3 py-1 rounded-full bg-green-50 text-green-800 border border-green-200 text-xs dark:bg-green-900/40 dark:text-green-300 dark:border-green-500">{soft}</li>
                          ))}
                        </ul>
                      </section>
                    )}
                    ...
                    {experiencias && experiencias.length > 0 && (
                      <section>
                        <h4 className="text-xl font-semibold text-gray-800 dark:text-gray-100 mb-1">Experiências</h4>
                        <div className="space-y-3">
                          {experiencias.map((xp, index) => (
                            <div key={index} className="rounded-xl bg-gray-50 dark:bg-gray-800 px-3 py-2 text-sm text-gray-800 dark:text-gray-100">
                              <p className="font-semibold text-lg">{xp.empresa}</p>
                              <p className="text-xs text-gray-600 dark:text-gray-300">{xp.cargo} • {xp.inicio} - {xp.fim}</p>
                    ...
                    {formacao && formacao.length > 0 && (
                      <section>
                        <h4 className="text-xl font-semibold text-gray-800 dark:text-gray-100 mb-1">Formação</h4>
                        <div className="space-y-2 text-sm text-gray-800 dark:text-gray-100">
                          {formacao.map((f, index) => (
                            <div key={index}>
                              ...
                              <p className="text-xs text-gray-600 dark:text-gray-300">{f.instituicao} • {f.ano}</p>
                            </div>
                          ))}
                        </div>
                      </section>
                    ...
                    {projetos && projetos.length > 0 && (
                      <section>
                        <h4 className="text-xl font-semibold text-gray-800 dark:text-gray-100 mb-1">Projetos</h4>
                        <div className="space-y-2 text-sm text-gray-800 dark:text-gray-100">
                          {projetos.map((pro, index) => (
                            <div key={index} className="rounded-xl bg-gray-50 dark:bg-gray-800 px-3 py-2">
                              <p className="font-semibold">{pro.titulo}</p>
                              {pro.link && (
                                <p className="text-xs text-green-700 dark:text-green-300 break-all">{pro.link}</p>
                              )}
                              <p className="text-xs mt-1 text-gray-700 dark:text-gray-300">{pro.descricao}</p>
                            </div>
                          ))}
                        </div>
                      </section>
                    ...
                    {certificacoes && certificacoes.length > 0 && (
                      <section>
                        <h4 className="text-xl font-semibold text-gray-800 dark:text-gray-100 mb-1">Certificações</h4>
                    ...
                    {idiomas && idiomas.length > 0 && (
                      <section>
                        <h4 className="text-xl font-semibold text-gray-800 dark:text-gray-100 mb-1">Idiomas</h4>
                        <div className="space-y-1 text-sm text-gray-800 dark:text-gray-100">
                          {idiomas.map((idio, index) => (
                            <p key={index}><span className="font-semibold">{idio.idioma}:</span> {idio.nivel}</p>
                          ))}
                        </div>
                      </section>
                    )}
                    ...
                    {areaInteresses && areaInteresses.length > 0 && (
                      <section>
                        <h4 className="text-xl font-semibold text-gray-800 dark:text-gray-100 mb-1">Áreas de interesse</h4>
                        <ul className="flex flex-wrap gap-2">
                          {areaInteresses.map((areaItem) => (
                            <li key={areaItem} className="px-3 py-1 rounded-full bg-green-50 text-green-800 border border-green-200 text-xs dark:bg-green-900/40 dark:text-green-300 dark:border-green-500">{areaItem}</li>
                          ))}
                        </ul>
                      </section>
                    )}
                    ...
                    {recomendacoes.length > 0 && (
                      <section className="mt-4">
                        <h4 className="text-xl font-semibold text-gray-800 dark:text-gray-100 mb-1">Recomendações</h4>
                        <div className="space-y-3">
