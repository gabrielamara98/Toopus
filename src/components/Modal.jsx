            ...
            <DialogPanel transition className="relative transform overflow-hidden rounded-3xl bg-white text-left shadow-2xl transition-all data-closed:translate-y-4 data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in sm:my-8 sm:w-full sm:max-w-3xl data-closed:sm:translate-y-0 data-closed:sm:scale-95">
                ...
                <div className="flex flex-col sm:flex-row gap-8">
                  ...
                  <div className="sm:w-2/3 max-h-[60vh] overflow-y-auto pr-1 space-y-4">
                    ...
                    {areaInteresses && areaInteresses.length > 0 && (
                      <section>
                        <h4 className="text-sm font-semibold text-gray-800 mb-1 text-xl">
                          Áreas de interesse
                        </h4>
                        <ul className="flex flex-wrap gap-2">
                    ...
                    {recomendacoes.length > 0 && (
                      <section className="mt-4">
                        <h4 className="text-sm font-semibold text-gray-800 mb-1 text-xl">Recomendações</h4>
                        ...
                        <div className="space-y-3">
                          {recomendacoes.map((rec, index) => (
                            <div
                              ...
                              <p className="mt-1 text-sm text-gray-800">{rec.mensagem}</p>
              ...
              <div className="bg-gray-50 px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full sm:w-auto">
                  ...
                  <button onClick={handleRecomendar} className="flex-1 rounded-full border border-green-600 text-green-700 px-4 py-2 text-sm font-medium hover:bg-green-600 hover:text-white transition-all">Recomendar</button>
                </div>

                <button type="button" onClick={() => setOpen(false)} className="inline-flex justify-center rounded-full bg-red-500 px-4 py-2 text-sm font-semibold text-white hover:bg-red-400">Sair</button>
              </div>
            </DialogPanel>
          </div>
        </div>
