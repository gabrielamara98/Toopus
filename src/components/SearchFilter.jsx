...
function SearchFilter({ onFilter }) {
  ...
  const estados = Array.from(
    new Set(
      dados
        .map((u) => u.localizacao.split("/")[1]?.trim())
        .filter(Boolean)
    )
  ).sort();

  function aplicarFiltros(nAreas, nIdiomas, nEstados) {
    let filtrados = dados;
    ...
    if (nEstados.length > 0) {
      filtrados = filtrados.filter((u) => {
        const estado = u.localizacao.split("/")[1]?.trim();
        return nEstados.includes(estado);
      });
    }
  ...
  function atualizar(e, lista, setLista) {
    ...
    aplicarFiltros(
      ...
      lista === estadosSelecionados ? novaLista : estadosSelecionados
    );
  }
      ...
      <aside className="w-full mt-4">
        <div className="bg-white border border-gray-200 rounded-3xl shadow-md px-6 py-6 dark:bg-gray-800 dark:border-gray-700">
          ...
          <section className="mb-2">
            <h2 className="text-base font-semibold text-gray-800 dark:text-gray-100 mb-3">Estado</h2>
            ...
            <div className="grid grid-cols-2 gap-y-2 text-sm text-gray-700 dark:text-gray-200">
              {estados.map((estado) => (
                <label key={estado} className="flex items-center gap-2">
                  <input type="checkbox" value={estado} className="accent-green-600 cursor-pointer" onChange={(e) =>
                      atualizar(e, estadosSelecionados, setEstadosSelecionados)}/>
                  <span>{estado}</span>
                </label>
              ))}
            </div>
